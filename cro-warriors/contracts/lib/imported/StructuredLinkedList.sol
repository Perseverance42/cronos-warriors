...
library StructuredLinkedList {
    ...
    function getLast(List storage self) internal view returns (uint256){
        return self.last;
    }
    ...
    function remove(List storage self, uint256 _node) internal returns (uint256) {
        ...
        if(_node==self.first){
            self.first = self.list[_node][_NEXT];
        }
        if(_node==self.last){
            self.last = self.list[_node][_PREV];
        }
    ...
    function _insert(List storage self, uint256 _node, uint256 _new, bool _direction) private returns (bool) {
        if (!nodeExists(self, _new) && nodeExists(self, _node)) {
            ...
            if(self.list[_new][_NEXT] == _NULL){
                self.last = _new;
            }
            if(self.list[_new][_PREV] == _NULL){
                self.first = _new;
            }
